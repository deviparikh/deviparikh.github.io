function setup() {
    ...
    var or = Number(document.getElementById("offsetRight").value);
    var ol = Number(document.getElementById("offsetLeft").value);
    // or = 110;
    // ol = -70;
    ...
    for (var trial = 0; trial < numTrials; trial++) {
        ...
        check = Math.random();
        if (check < 0.5) {
            cp1[0] = (p[0] + pprev[0]) / 2 + ol;
            cp1[1] = (p[1] + pprev[1]) / 2;
            cp2[0] = (p[0] + pprev[0]) / 2 - or;
            cp2[1] = (p[1] + pprev[1]) / 2;
        } else {
            cp1[0] = (p[0] + pprev[0]) / 2 - ol;
            cp1[1] = (p[1] + pprev[1]) / 2;
